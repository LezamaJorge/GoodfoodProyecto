...
class MyCartController extends GetxController {
  RestaurantDetailScreenController detailScreenController = Get.put(RestaurantDetailScreenController());
  ...
  double calculateFinalAmount() {
    ...
    try {
      ...
      double amountAfterDiscount = subTotal - couponAmount.value;

      for (var taxes in taxList) {
      ...
      finalAmount = amountAfterDiscount + taxAmount + deliveryFee.value;
    } catch (e, stackTrace) {
  ...
  Future<void> completeOrder(String transactionId, bool? isPayment) async {
    try {
      ...
      //    = selectedPaymentMethod.value == Constant.paymentModel!.cash!.name ? false : true;
      // }
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response, String amount) {
    try {
      ShowToastDialog.showToast("Payment processed successfully.");
  ...
  void _handlePaymentError(PaymentFailureResponse response) {
    try {
      ShowToastDialog.showToast('Payment could not be completed. Please try again.');
    } catch (e) {
      ...
      if (response.statusCode == 200) {
        ...
        if (result != null && result is Map<String, dynamic>) {
          if (result["status"] == true) {
            ...
            ShowToastDialog.showToast("Payment was unsuccessful.");
          }
        } else {
          ShowToastDialog.showToast("Payment was unsuccessful.");
        }
        ...
        if (response.statusCode == 400) {
          ...
          ShowToastDialog.showToast("Failed to initiate payment. Please try again.");
        }
      }
    } catch (e, stackTrace) {
  ...
  Future<void> payStackPayment({required String totalAmount}) async {
    try {
      ...
      if (payStackModel != null) {
        ...
        ShowToastDialog.showToast("An error occurred. Please contact support.");
      }
    } catch (e, stackTrace) {
      ...
      if (result.isNotEmpty) {
        if (result['status'] == 200) {
          ...
          if (value == true) {
            ...
            ShowToastDialog.showToast("Payment could not be completed.");
          }
        } else {
          ShowToastDialog.showToast("Transaction error occurred.");
        }
      } else {
        ShowToastDialog.showToast("Transaction error occurred.");
      }
  ...
  Future<void> payFastPayment({required BuildContext context, required String amount}) async {
    try {
      ...
      if (htmlData.isNotEmpty) {
        ...
        ShowToastDialog.showToast("Unable to load payment gateway.");
      }
    } catch (e, stackTrace) {
