...
class FireStoreUtils {
  ...
  static Future<List<ProductModel>> getProductsFromVendorsWithSearch({
    ...
    List<ProductModel> productList = [];
    try {
      if (vendorIds.isEmpty) return productList;

      var products = await FirebaseFirestore.instance.collection(CollectionName.product).where("vendorId", whereIn: vendorIds).get();
      ...
      for (var element in products.docs) {
        ...
        if (words.any((word) => word.contains(query.toLowerCase()))) {
          productList.add(product);
        }
      }
    } catch (e) {
      developer.log('Error fetching products with search: $e');
    }
    return productList;
  }
