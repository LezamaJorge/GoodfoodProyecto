        ...
        if (order.createdAt != null) {
          ...
          if (orderDate.year == currentYear) {
            ...
            if (orderDate.isAfter(weekStartDate) &&
                orderDate.isBefore(todayDate.add(Duration(days: 1)))) {
    ...
    try {
      ...
      if (selectedTimeTotalOrders.value == 'Monthly') {
        ...
        rejectedOrdersCount = rejectOrderList.where((order) {
          ...
          return orderTimestamp.compareTo(startOfYearTimestamp) >= 0 &&
              orderTimestamp.compareTo(endOfYearTimestamp) <= 0;
        }).length;

        completedOrdersCount = orderCompletedList.where((order) {
          Timestamp orderTimestamp = order.createdAt!;
          return order.orderStatus == 'order_complete' &&
              orderTimestamp.compareTo(startOfYearTimestamp) >= 0 &&
              orderTimestamp.compareTo(endOfYearTimestamp) <= 0;
        }).length;

        cancelledOrdersCount = rejectOrderList.where((order) {
          Timestamp orderTimestamp = order.createdAt!;
          return order.orderStatus == 'order_cancel' &&
              orderTimestamp.compareTo(startOfYearTimestamp) >= 0 &&
              orderTimestamp.compareTo(endOfYearTimestamp) <= 0;
        }).length;
        ...
        rejectedOrdersCount = rejectOrderList.where((order) {
          Timestamp orderTimestamp = order.createdAt!;
          return orderTimestamp.compareTo(weekStartTimestamp) >= 0 &&
              orderTimestamp.compareTo(weekEndTimestamp) <= 0;
        }).length;

        completedOrdersCount = orderCompletedList.where((order) {
          Timestamp orderTimestamp = order.createdAt!;
          return order.orderStatus == 'order_complete' &&
              orderTimestamp.compareTo(weekStartTimestamp) >= 0 &&
              orderTimestamp.compareTo(weekEndTimestamp) <= 0;
        }).length;
